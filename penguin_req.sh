...
hadoop fs -getmerge ${output_dir}/part* data/penguin_req_${day}${hour}
hadoop fs -rm -r ${output_dir}
cat data/penguin_req_${day}${hour} | python penguin_pingback.py
del_day=`date -d "-4 hours" +"%Y%m%d"`
del_hour=`date -d "-4 hours" +"%H"`
del_file=data/penguin_req_${del_day}${del_hour}
rm -f ${del_file}
